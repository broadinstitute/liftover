...
Every `make deploy` pushes a new image, and nothing used to remove the old ones, so the shared
us-central1-docker.pkg.dev/spliceai-lookup-412920/docker repo had grown to 217 images / 278 GB by
2026-09-01. `make all` runs this after deploying so the repo stays small on its own.

This registry repo is shared with SpliceAI-lookup, whose spliceai/pangolin images are pruned by
its own copy of this file at google_cloud_run_services/cleanup_old_images.py. The two copies are
the same script; change one and change the other. Each deploy only ever prunes its own --package,
so the two never touch each other's images.
...
Examples:
  ./cleanup_old_images.py --package liftover
  ./cleanup_old_images.py --package liftover --keep 5
...
def digests_recorded_in_repo():
    ...
    Globbed rather than hard-coded so the copy of this script in the SpliceAI-lookup repo (see
    the module docstring) needs no change: this repo keeps a single sha256.txt and that one
    keeps docker/<tool>/sha256_grch<hg>.txt, and both sit under the directory holding this
    script. Missing files are not an error -- a checkout that never deployed has none.
    """
    digests = set()
